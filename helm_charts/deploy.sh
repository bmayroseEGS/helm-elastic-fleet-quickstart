...
main() {
    ...
    echo ""
    echo -e "${GREEN}Step 1: Run the Fleet setup script${NC}"
    echo ""
    echo -e "  ${YELLOW}cd ../deployment_infrastructure${NC}"
    echo -e "  ${YELLOW}./setup-fleet.sh${NC}"
    echo ""
    echo "  While the script is running, you will see:"
    echo "  ${BLUE}Checking Kibana availability at http://localhost:5601...${NC}"
    echo "  ${BLUE}  Waiting for Kibana... (attempt 1/30)${NC}"
    echo "  ${BLUE}  Waiting for Kibana... (attempt 2/30)${NC}"
    echo "  ${BLUE}  ...${NC}"
    echo ""
    echo -e "${GREEN}Step 2: Setup port forwarding (from another terminal)${NC}"
    echo ""
    echo "  While the setup-fleet.sh script is waiting for Kibana,"
    echo "  open a NEW terminal and create an SSH tunnel with port forwarding:"
    echo ""
    echo -e "  ${YELLOW}ssh -i your-key.pem -L 9200:localhost:9200 -L 5601:localhost:5601 -L 8080:localhost:8080 user@server${NC}"
    echo ""
    echo "  Once logged in to the remote server, run these kubectl port-forward commands:"
    echo ""
    echo -e "  ${YELLOW}kubectl port-forward -n elastic svc/elasticsearch-master 9200:9200 &${NC}"
    echo -e "  ${YELLOW}kubectl port-forward -n elastic svc/kibana 5601:5601 &${NC}"
    echo -e "  ${YELLOW}kubectl port-forward -n elastic svc/logstash 5044:5044 &${NC}"
    echo ""
    echo "  Note: The '&' runs commands in background. This allows all three port-forwards"
    echo "        to run simultaneously in the same terminal."
    echo ""
    echo -e "${GREEN}Step 3: The setup-fleet.sh script will continue${NC}"
    echo ""
    echo "  Once port-forwarding is active, the setup-fleet.sh script will:"
    echo "    • Detect Kibana is ready"
    echo "    • Configure Fleet in Kibana via API"
    echo "    • Deploy Fleet Server Helm chart"
    echo "    • Wait for Fleet Server to be ready"
    echo "    • Display the Fleet UI URL"
    echo ""
    echo -e "${GREEN}Step 4: Access Fleet Server${NC}"
    echo ""
    echo "  Navigate to: ${GREEN}http://localhost:5601/app/fleet/agents${NC}"
    echo ""
    echo "  Login with:"
    echo "    Username: elastic"
    echo "    Password: elastic"
    echo ""
    echo "  You will see the Fleet Server running and ready to enroll agents!"
    echo ""
